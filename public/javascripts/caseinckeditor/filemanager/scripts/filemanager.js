...
var treeConnector = '/casein/ckeditor/folders';
//var fileConnector = 'connectors/' + lang + '/filemanager.' + lang;
var fileConnector = '/casein/ckeditor/connector';
...
var deleteItem = function(data){
	...
	var doDelete = function(v, m){
		...
		if(data['File Type'] == 'dir')
			...
			var connectString = fileConnector + '?mode=delete&path=' + data['Parent ID'] + '/' + data['ID'];
		...
		$.ajax({
			...
			success: function(result){
				if(result['Code'] == 0){
					removeNode(result['Path'], result['Parent']);
					isDeleted = true;
...
// delete operation.
var removeNode = function(path, parent){
	$('#filetree')
		.find('a[rel="' + path + '"]')
		.parent()
...
var addFolder = function(parent, name, id){
	if(parent == "/") {
		createFileTree();
		return;
	}
	
	var newNode = '<li class="directory collapsed"><a rel="' + parent + '/' + id + '" href="#">' + name + '/</a><ul class="jqueryFileTree" style="display: block;"></ul></li>';
...
var setMenus = function(action, path){
	var url = fileConnector + '?mode=getinfo&path=' + path;
	if($.urlParam('kind') == 'image') {
		url = fileConnector + '?mode=getinfo&style=' + $.urlParam('style') + '&path=' + path;
	}
...
var getFileInfo = function(file){
	...
	var url = fileConnector + '?mode=getinfo&path=' + file;
	if($.urlParam('kind') == 'image') {
		url = fileConnector + '?mode=getinfo&style=' + $.urlParam('style') + '&path=' + file;
	}
	$.getJSON(url, function(data){
		if(data['Code'] == 0){
...
var getFolderInfo = function(path){
	...
	$.getJSON(fileConnector + '?kind=' + $.urlParam('kind') + '&path=' + path + '&mode=getfolder&showThumbs=' + showThumbs, function(data){		
		var result = '';
...
$(function(){
	...
	$("#kind").val($.urlParam('kind'));
	$('#uploader').attr('action', fileConnector);

	$('#uploader').ajaxForm({
		target: '#uploadresponse',
		beforeSubmit: function(arr, $form, options) {
			if($('#currentpath').val() == "" || $('#currentpath').val() == '/') {
				$.prompt("Please select a folder first and then upload again.");
				return false;
			}
			if($("#newfile").val() == "") {
				$.prompt("Select a file first.");
				return false;
			}
			return true;
		},
